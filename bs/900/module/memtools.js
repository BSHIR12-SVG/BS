...
import { get_view_vector } from './memtools.js'; // Fallback متبادل للربط الافتراضي
...
export function init_syscall_array(
    ...
    // استكمال ومطابقة مصفوفة السيسكال الثنائية المقطوعة لمعالجات x86_64
    // syscall_X:
    //     mov rax, X     -> 48 c7 c0 X XX XX XX
    //     mov r10, rcx   -> 4c 89 ca
    //     syscall        -> 0f 05
    //     ret            -> c3
    for (let i = 0; i < text_size; i++) {
        if (kbuf[i] === 0x48
            ...
            // استخراج رقم استدعاء النظام الموحد ثنائياً من الـ Opcode الممرر
            const sys_num = kbuf[i + 3] | (kbuf[i + 4] << 8) | (kbuf[i + 5] << 16) | (kbuf[i + 6] << 24);
            const sys_addr = libkernel_web_base.add(i);
            
            // تسجيل مسار الاستدعاء المباشر بداخل مصفوفة النظام الكلية للـ Host
            syscall_array[sys_num] = sys_addr;
        }
    }
    return true;
}
