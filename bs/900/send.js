/* Copyright (C) 2024-2026 anonymous
...
function dump(name, lib_base, lib_end) {
    ...
    log(`[DUMP] ${name} base: ${lib_base}`);
    log(`[DUMP] ${name} size: ${lib_size}`);
    
    const lib = make_buffer(lib_base, lib_size);
    
    // تأمين الإرسال البرمجي وفحص التوصيل مع مخدم المطور لمنع انهيار متصفح الـ PS4
    try {
        send(
            url,
            lib,
            `${name}.sprx.text_${lib_base}.bin`,
            () => log(`✔ تم سحب وإرسال ملف: ${name} بنجاح.`)
        );
    } catch(e) {
        log(`❌ فشل إرسال الحزمة للسيرفر: ${e.message}`);
    }
}

export function dump_libwebkit() {
    ...
    try {
        send(url, vtable, `vtable_${addr}.bin`, () => log('✔ تم سحب الـ vtable بنجاح.'));
    } catch(e) { }
...
export function dump_libkernel(libwebkit_base) {
    const offset = 0x8d8;
...
export function dump_libc(libwebkit_base) {
    const offset = 0x918;
...
// الدالة التنفيذية الكلية لسحب كامل مكتبات متصفح سوني 4 دفعة واحدة بشكل محمي
export function execute_complete_dump() {
    log("🚀 جاري بدء عملية الفحص وتفريغ حزم النظام الحساسة...");
    try {
        const libwebkit_base = dump_libwebkit();
        dump_libkernel(libwebkit_base);
        dump_libc(libwebkit_base);
        log("✔ اكتملت عملية سحب ملفات النظام بالكامل.");
    } catch (err) {
        log(`❌ تعذر إكمال السحب التلقائي: ${err.message}`);
    }
}

export function dump_eval() {
    let addr = js_textarea;
    addr = addr.readp(0x18);
    addr = addr.readp(0);
    ...
    try {
        send(
            ...
            () => log('✔ تم سحب الـ Eval بنجاح.')
        );
    } catch(e) { }
}

export function dump_scrollLeft() {
    ...
    try {
        send(
            ...
            () => log('✔ تم سحب الـ scrollLeft بنجاح.')
        );
    } catch(e) { }
}
