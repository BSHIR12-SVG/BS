...
// تعريف حجم الصفحة الثابت لمعالجات أجهزة البلايستيشن 4 لمنع أخطاء الذاكرة
...
function load_bin(data, exit) {
  // محاذاة حجم البيانات الممررة لـ GoldHEN بناءً على مساحة الذاكرة الافتراضية
  const sz = (typeof data.length.alignUp === "function") 
             ? data.length.alignUp(PAGE_SIZE) 
             : Math.ceil(data.length / PAGE_SIZE) * PAGE_SIZE;
  ...
  if (exit) {
    // تأمين دالة إغلاق وقتل عملية المتصفح الأبوية بنمط مستقر يمنع تجمد الشاشة
    fn.kill = new NativeFunction(0x25, "number");
    const pid = fn.getpid.invoke();
    
    logger.info(`Exiting browser process securely for PID: ${pid}`);
