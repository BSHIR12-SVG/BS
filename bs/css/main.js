...
async function doJb() {
  ...
  try {
    ...
    if (fn.setuid.invoke(0) !== -1) {
      // توحيد المظهر والرسائل المتناسقة مع النجاح باللون الأخضر المضيء
      const m = document.getElementById("msgs");
      if (m) {
        m.innerHTML = "✔ تم تحميل GoldHEN بنجاح سابقاً";
        m.style.color = "#4cd137";
      }
      return;
    }
    ...
    // إرسال إشعار النجاح النهائي الموحد باللون الأخضر النيون المضيء
    const m = document.getElementById("msgs");
    if (m) {
      m.innerHTML = "✔ تم تحميل GoldHEN بنجاح.. استمتع بالألعاب 🎮";
      m.style.color = "#4cd137";
    }
    logger.info("===END===");
  } catch (e) {
    // إرسال رسالة الفشل الموحدة بالأحمر النيون بدلاً من الأصفر المتضارب مع التصميم الداكن
    const m = document.getElementById("msgs");
    if (m) {
      m.innerHTML = "❌ فشل تفعيل الثغرة! أعد تشغيل جهازك وحاول مجدداً";
      m.style.color = "#ff4757";
    }
    logger.error("Exploit Error: " + e.message);
  }
}
