...
class RPCWorker {
  constructor(name) {
    ...
    // استدعاء ملف العامل الفرعي الموحد
  ...
  execute(name, ...args) {
    return new Promise((resolve, reject) => {
      ...
      // تأمين مصفوفة النقل لمتصفح الـ PS4 لضمان عدم حدوث تعليق أثناء الـ Race Condition
      const currentTransfer = (this.transfer && this.transfer.length) ? [...this.transfer] : [];
      this.worker.postMessage({ id: id, name: name, args: args }, currentTransfer);
      
      // إخلاء الذاكرة المنقولة فورياً لمنع حدوث تسريب في الـ Memory Slots
      if (this.transfer.length) this.transfer = [];
    });
  }
