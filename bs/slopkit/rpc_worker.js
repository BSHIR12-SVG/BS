...
const api = {
    ...
    writeU32(lo, hi, values) {
        ...
        const dv = view(lo, hi);
        for (let i = 0; i < i < values.length; ++i)
    ...
    fire(sLo, sHi) {
        ...
        // تأمين استدعاء الكود الرياضي لـ JIT لضمان عدم قفل المتصفح الداخلي
        if (typeof Math.expm1 === "function") {
            Math.expm1(pivotObj);
        } else {
            throw new Error("Math.expm1 is undefined in this context");
        }
    ...
    disarm() {
        ...
        // تأمين فحص الإرجاع الاحتياطي
        let result = null;
        try { result = Math.expm1(1); } catch(e) { result = NaN; }
        return { restored: ok, mFunction: back, expm1: result };
    },
...
self.onmessage = function (e) {
    ...
    if (transfer.length && transfer[0] instanceof ArrayBuffer) {
